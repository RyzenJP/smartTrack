<?php
session_start();
// Allow Motor Pool Admin to access geofencing (view-only)
        ...
        <div class="card shadow-sm">
          <div class="card-body p-3">
            ...
            <h5 class="card-title text-primary fw-bold m-0">Geofencing</h5>
              <!-- Drawing tools removed for Motor Pool Admin view-only mode -->
            ...
            <!-- Geofencing Features instructions removed for Motor Pool Admin view-only mode -->
            
            <div class="row">
              <div class="col-md-8">
            <div id="map"></div>
                <div class="action-buttons mt-2">
                  <!-- View-only mode - editing disabled for Motor Pool Admin -->
                  <div class="alert alert-info p-2 mb-0">
                    <i class="fas fa-info-circle me-2"></i>
                    <strong>View Mode:</strong> You can view geofences. Editing is restricted to Super Admins only.
                  </div>
            </div>
          </div>
              <div class="col-md-4">
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (typeof L === 'undefined') {
    ...
    document.getElementById('map').innerHTML = '<div class="alert alert-danger">Map library not loaded. Please refresh the page.</div>';
    return;
  }
  ...
  let map;
  try {
    map = L.map('map').setView([10.5388, 122.8388], 14);
    console.log('Map initialized successfully');
  } catch (error) {
    console.error('Map initialization failed:', error);
    document.getElementById('map').innerHTML = '<div class="alert alert-danger">Failed to load map. Please refresh the page.</div>';
    return;
  }
  ...
  // View-only mode - no drawing tools needed
  ...
  // Tool buttons removed for Motor Pool Admin view-only mode
  // Drawing tools are not available in view-only mode

    // Update cursor (view-only mode - no drawing)
  ...
  function clearTempDrawing() {
    ...
    // Hide finalize button (if it exists)
    const finalizeBtn = document.getElementById('finalizeRectangleBtn');
    if (finalizeBtn) finalizeBtn.style.display = 'none';
  }

  // Tool event listeners removed for Motor Pool Admin view-only mode
  // Drawing tools are not available in view-only mode

  // Action button event listeners removed for Motor Pool Admin view-only mode
  // Drawing and editing functionality is not available in view-only mode

  // Auto-start monitoring on page load
  console.log('Starting auto-monitoring...');

  // Map click handler removed for Motor Pool Admin view-only mode
  // Drawing functionality is not available in view-only mode

  // Double-click and right-click handlers removed for Motor Pool Admin view-only mode
  // Drawing functionality is not available in view-only mode

  // Keyboard shortcuts removed for Motor Pool Admin view-only mode
  // Drawing functionality is not available in view-only mode

  function createCircleGeofence(center) {
    // Drawing functions not used in view-only mode
  ...
  // Save geofence (check if element exists first)
  const confirmSaveBtn = document.getElementById('confirmSaveGeofence');
  if (confirmSaveBtn) {
    ...
    try{
      ...
      if(result.success){
        ...
        // Reset drawing mode (not needed in view-only mode)
  ...
  // Clear drawing button (check if element exists first)
  // Clear drawing button removed for view-only mode

  // DISABLED IN VIEW-ONLY MODE - Delete functionality removed
  /*
  document.getElementById('deleteGeofenceBtn').addEventListener('click', async function(){
    if(!currentGeofence?.id){ Swal.fire('Error','No geofence selected','error'); return; }
    const { isConfirmed } = await Swal.fire({
      title:'Delete this geofence?', icon:'warning', showCancelButton:true,
      confirmButtonColor:'#dc3545', cancelButtonColor:'#6c757d', confirmButtonText:'Delete'
    });
    if(isConfirmed){
      try{
        const resp = await fetch(`geofence_api.php?action=delete_geofence&id=${currentGeofence.id}`);
        const res = await resp.json();
        if(res.success){ 
          Swal.fire('Deleted','Geofence deleted','success'); 
          
          // Remove from map
          const layer = geofenceLayers.get(currentGeofence.id);
          if (layer) {
            map.removeLayer(layer);
            geofenceLayers.delete(currentGeofence.id);
          }
          
          currentGeofence = null;
          loadGeofences();
        }
      }catch(e){ console.error(e); Swal.fire('Error','Failed to delete','error'); }
    }
  });
  */
  ...
  function updateGeofenceList() {
    ...
    listContainer.innerHTML = geofences.map(geofence => `
      <div class="d-flex justify-content-between align-items-center p-2 border-bottom" 
        ...
        <!-- Edit and Delete buttons removed for Motor Pool Admin view-only mode -->
      </div>
    `).join('');
  }
  ...
  async function loadGeofences(){
    try{
      ...
      const result = await resp.json();
      console.log('Geofences response:', result);
      ...
      if(success){
        ...
        console.error('Failed to load geofences:', result);
        updateGeofenceList([]);
        // Show error message in geofence list
        document.getElementById('geofenceList').innerHTML = '<div class="alert alert-warning p-2">Failed to load geofences. Please try again.</div>';
      }
    }catch(e){ 
      console.error('Error loading geofences:', e); 
      updateGeofenceList([]);
      // Show error message in geofence list
      document.getElementById('geofenceList').innerHTML = '<div class="alert alert-danger p-2">Error loading geofences: ' + e.message + '</div>';
    }
  }
  ...
  // DISABLED IN VIEW-ONLY MODE - Edit functionality removed
  /*
  window.editGeofence = async function(id) {
    ...
    if (geofence) {
      ...
      // Populate the save modal with current values
      ...
      document.getElementById('geofenceRadius').value = geofence.radius || 500;
  ...
  */
  ...
  window.deleteGeofence = async function(id) {
    ...
    if (isConfirmed) {
      try {
        const resp = await fetch(`geofence_api.php?action=delete_geofence&id=${id}`);
        const result = await resp.json();
        ...
        if (result.success) {
          ...
          // Remove from map
          const layer = geofenceLayers.get(id);
          if (layer) {
            map.removeLayer(layer);
          ...
          Swal.fire('Error!', result.message || 'Failed to delete geofence', 'error');
        }
      } catch (error) {
  ...
  */
  ...
  function startAutoMonitoring() {
    ...
    if (lastUpdate) {
      ...
      console.log('Last update time set to:', lastUpdate.textContent);
    }
  ...
  function isVehicleInGeofence(vehicle, geofence) {
    ...
    if (geofence.type === 'circle') {
      ...
      return isPointInPolygon([vehicleLat, vehicleLng], geofence.polygon);
  ...
  function updateMonitoringUI(vehicleCount = null) {
    ...
    // Update last update time to show it's running
    const now = new Date();
    document.getElementById('lastUpdate').textContent = now.toLocaleTimeString();
    document.getElementById('lastUpdate').className = 'text-success'; // Make it green to show it's active
