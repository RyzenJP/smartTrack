...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (isset($_POST['action'])) {
            switch ($_POST['action']) {
                case 'add':
                    // Add new dispatcher using MySQLi
                    $stmt = $conn->prepare("
                    ...
                    $stmt->execute();
                    $_SESSION['success'] = 'Dispatcher added successfully';
                    break;
                    
                case 'edit':
                    // Update dispatcher using MySQLi
                    $stmt = $conn->prepare("
                    ...
                    $stmt->execute();
                    $_SESSION['success'] = 'Dispatcher updated successfully';
                    break;
...
// Lightweight AJAX endpoint to check username availability
if (isset($_GET['action']) && $_GET['action'] === 'check_username') {
    header('Content-Type: application/json');
    $username = trim($_GET['username'] ?? '');
    if ($username === '') { echo json_encode(['ok' => false, 'message' => 'Empty username']); exit; }
...
// Fetch all dispatcher users using MySQLi - use prepared statement for consistency
$stmt = $conn->prepare("
...
<head>
  ...
  <title>Dispatcher Management | Smart Track</title>
...
<body>
      ...
      <div class="card shadow-sm">
        <div class="card-body">
          <div class="d-flex justify-content-between align-items-center mb-3">
            <h5 class="card-title text-primary fw-bold">Dispatcher Accounts</h5>
            <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addDispatcherModal">
              <i class="fas fa-plus me-1"></i> Add Dispatcher
            </button>
          </div>
  ...
  <!-- Add Dispatcher Modal - Landscape Version -->
<div class="modal fade" id="addDispatcherModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-landscape">
    <div class="modal-content">
      <form method="POST" action="dispatcher.php" id="addDispatcherForm" novalidate>
          ...
          <div class="row">
            <div class="col-md-6">
              ...
              <div class="mb-3">
                ...
                <input type="text" class="form-control" name="username" id="dispatcherUsername" required minlength="4">
            ...
            <div class="col-md-6">
              <div class="mb-3">
                ...
                <input type="tel" class="form-control" name="phone" id="dispatcherPhone" pattern="[0-9]{11}" maxlength="11" inputmode="numeric" required>
                <div class="form-text" id="phoneFeedback">Must be 11 digits only</div>
              </div>
              <div class="mb-3">
                <label class="form-label">Password <span class="text-danger">*</span></label>
                <div class="position-relative">
                  <input type="password" class="form-control" name="password" id="dispatcherPassword" required minlength="8" style="padding-right: 40px;">
                  <button class="btn btn-link position-absolute" type="button" id="toggleAddPassword" style="top: 50%; right: 8px; transform: translateY(-50%); padding: 0; background: none; border: none; color: #6c757d; z-index: 5;">
                    <i class="fas fa-eye" id="addPasswordIcon"></i>
                  </button>
                </div>
                <div class="form-text">Min 8 chars, use upper, lower, number</div>
                <div class="progress mt-2" style="height: 6px;">
                  <div id="dispatcherPwdStrength" class="progress-bar" role="progressbar" style="width:0%"></div>
                </div>
              </div>
  ...
  <div class="modal fade" id="editDispatcherModal" tabindex="-1" aria-hidden="true" style="z-index: 9999;">
            ...
            <div class="row">
              ...
              <div class="col-md-6">
                ...
                <div class="mb-3">
                  ...
                  <div class="position-relative">
                    <input type="password" class="form-control" name="password" id="edit_password" minlength="8" style="padding-right: 40px;">
                    <button class="btn btn-link position-absolute" type="button" id="toggleEditPassword" style="top: 50%; right: 8px; transform: translateY(-50%); padding: 0; background: none; border: none; color: #6c757d; z-index: 5;">
                      <i class="fas fa-eye" id="editPasswordIcon"></i>
                    </button>
                  </div>
                  <div class="form-text text-muted">Min 8 chars, use upper, lower, number</div>
                  <div class="progress mt-2" style="height: 6px;">
                    <div id="editPwdStrength" class="progress-bar" role="progressbar" style="width:0%"></div>
                  </div>
                </div>
              </div>
            </div>
  ...
  <script>
    ...
    document.querySelectorAll('.edit-btn').forEach(btn => {
      btn.addEventListener('click', function() {
        ...
        const modal = new bootstrap.Modal(document.getElementById('editDispatcherModal'));
        modal.show();
      });
    });
    ...
    const addForm = document.getElementById('addDispatcherForm');
    const usernameInput = document.getElementById('dispatcherUsername');
    const usernameFeedback = document.getElementById('usernameFeedback');
    const phoneInput = document.getElementById('dispatcherPhone');
    const phoneFeedback = document.getElementById('phoneFeedback');
    const passwordInput = document.getElementById('dispatcherPassword');
    const strengthBar = document.getElementById('dispatcherPwdStrength');
    ...
    passwordInput?.addEventListener('input', () => {
      ...
      if (strengthBar) {
        ...
        strengthBar.className = 'progress-bar ' + (score < 40 ? 'bg-danger' : score < 70 ? 'bg-warning' : 'bg-success');
      }
    });

    // Password strength for edit modal
    const editPasswordInput = document.getElementById('edit_password');
    const editStrengthBar = document.getElementById('editPwdStrength');

    editPasswordInput?.addEventListener('input', () => {
      const val = editPasswordInput.value || '';
      const score = evaluateStrength(val);
      if (editStrengthBar) {
        editStrengthBar.style.width = score + '%';
        editStrengthBar.className = 'progress-bar ' + (score < 40 ? 'bg-danger' : score < 70 ? 'bg-warning' : 'bg-success');
      }
    });
    ...
    addForm?.addEventListener('submit', (e) => {
      ...
      // Username availability must not be taken
      ...
      if (phoneInput && !/^[0-9]{11}$/.test(phoneInput.value)) {
        e.preventDefault();
        e.stopPropagation();
      ...
      if (passwordInput && !(/[A-Z]/.test(passwordInput.value) && /[a-z]/.test(passwordInput.value) && /\d/.test(passwordInput.value))) {
        e.preventDefault();
        e.stopPropagation();
    ...
    phoneInput?.addEventListener('input', () => {
      ...
      if (numericValue.length === 0) {
        ...
        phoneInput.setCustomValidity('Phone must be 11 digits');
      } else if (numericValue.length > 11) {
    ...
    usernameInput?.addEventListener('input', () => {
      ...
      usernameTimer = setTimeout(async () => {
        try {
          const resp = await fetch(`dispatcher.php?action=check_username&username=${encodeURIComponent(value)}`, { cache: 'no-store' });
  ...
  document.addEventListener('DOMContentLoaded', function() {
    var addDispatcherModal = document.getElementById('addDispatcherModal');
    var editDispatcherModal = document.getElementById('editDispatcherModal');
    
    // Fix positioning for Add Dispatcher Modal
    addDispatcherModal.addEventListener('shown.bs.modal', function () {
        var navbarHeight = document.querySelector('.navbar').offsetHeight;
        this.style.marginTop = navbarHeight + 'px';
    });
    
    // Fix positioning for Edit Dispatcher Modal
    editDispatcherModal.addEventListener('shown.bs.modal', function () {
        var navbarHeight = document.querySelector('.navbar').offsetHeight;
        this.style.marginTop = navbarHeight + 'px';
    });
    ...
    const addPasswordInput = document.getElementById('dispatcherPassword');
    const addPasswordIcon = document.getElementById('addPasswordIcon');
    
    if (toggleAddPasswordBtn) {
      toggleAddPasswordBtn.addEventListener('click', function() {
        if (addPasswordInput.type === 'password') {
          addPasswordInput.type = 'text';
          addPasswordIcon.classList.remove('fa-eye');
          addPasswordIcon.classList.add('fa-eye-slash');
        } else {
          addPasswordInput.type = 'password';
          addPasswordIcon.classList.remove('fa-eye-slash');
          addPasswordIcon.classList.add('fa-eye');
        }
      });
    }
